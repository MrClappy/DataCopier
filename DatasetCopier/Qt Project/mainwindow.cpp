...
void MainWindow::setupCopier()
{
    copier = new QtFileCopier();
    dialog = new QtCopyDialog();
    dialog->setFileCopier(copier);
}

void MainWindow::cleanUpCopier()
{
    delete copier;
    delete dialog;
}
...
void MainWindow::readResult()
{
    ...
    if ( res.contains(QRegExp("bytes=32")) ) {
        ...
        // Get Folder List
        QDir dataFolders(QDir::fromNativeSeparators("//" + ui->IP_Box->text() + "/Data"));
...

{
    if (ui->copyButton->text() == "Copy") {
        ...
        if (ui->dataSetWidget->selectedItems().isEmpty()) {
            ...
            ui->status->setText("Starting... Please Wait");
            QCoreApplication::processEvents();
            ui->copyButton->setText("Cancel");

            if (ui->mergeButton->isChecked()) {
                QCoreApplication::processEvents();
                const QString rootPath = "//" + ui->IP_Box->text() + "/Data/";
                ...
                foreach (QListWidgetItem* item, items) {
                    ...
                    copyFolderFancy(fromPath, toPath);
