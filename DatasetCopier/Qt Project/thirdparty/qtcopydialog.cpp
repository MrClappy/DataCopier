...
QString millisecondsToFormattedTime(int milliseconds)
{
    ...
    QString time = QString::number(hours) + ":" + QString::number(minutes) + ":" + QString::number(seconds);
...
void QtCopyDialogPrivate::setCompleted(qint64 completed, qint64 totalSize, int msecs)
{
    ...
    if (completed > 0) {
        ...
        estStr = millisecondsToFormattedTime(estMSecs);
    }
    str = QtCopyDialog::tr("%1/s ( %2 remaining )").arg(transferStr).arg(estStr);
    ui.remainingLabel->setText(str);
  //  qDebug() << "totalSize: " << totalSize << endl << "msecs: " << msecs << endl << "completed: " << completed << endl;
}
